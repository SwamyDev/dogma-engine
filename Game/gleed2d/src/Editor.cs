...
namespace GLEED2D
{
    ...
    class Editor
    {
        ...
        private PolygonPath _currentPolygon;

        public bool physicsItemMode = false;
        public bool staticItemMode = false;
        public bool lightItemMode = false;
        ...
        // Bernhard's Hack
        public List<PhysicsPath> PhysicObjects = new List<PhysicsPath>();
        public Vector2 PhysicsExtentTopLeft = new Vector2(float.PositiveInfinity, float.PositiveInfinity);
        public Vector2 PhysicsExtentBottomRight = new Vector2(float.NegativeInfinity, float.NegativeInfinity);
        ...
        public void deleteSelectedItems()
        {
            ...
            foreach (Item selitem in selecteditemscopy) {
                ...
                // Remove game objects from dynamic list for drop down list in referencing properties -> Look at GameObjectReference class.
                if (selitem is GameObjectItem)
                    Scene.GameObjectNames.Remove(selitem.Name);
        ...
        public void AddGameObject()
        {
            ...
            Item i = new GameObjectItem(new Vector2((int)mouseworldpos.X, (int)mouseworldpos.Y));

            i.Name = i.getNamePrefix() + level.getNextItemNumber();
            i.layer = SelectedLayer;
            Scene.GameObjectNames.Add(i.Name);
        ...
        public void createTextureBrush(string fullpath, Type customEntityType)
        {
            // ZoidBerg
            this.customEntity = true;
            this.customEntityType = customEntityType;
            state = EditorState.brush;
            currentbrush = new Brush(fullpath, customEntityType);
        }
        ...
        public void paintTextureBrush(bool continueAfterPaint)
        {
            ...
            if (physicsItemMode)
            {
                ...
                if (staticItemMode)
                {
                    (i as PObjectTemplate).PhysicsType = "Static";
                }

            }
            else if (lightItemMode)
            {
                i = new LObjectTemplate(currentbrush.fullpath, new Vector2((int)mouseworldpos.X, (int)mouseworldpos.Y));
            }
            else if (customEntityType == typeof(PlayerStart))
            {
                i = new PlayerStart(currentbrush.fullpath, new Vector2((int)mouseworldpos.X, (int)mouseworldpos.Y));
            }
            else if (customEntityType == typeof(GuardStart))
            {
                i = new GuardStart(currentbrush.fullpath, new Vector2((int)mouseworldpos.X, (int)mouseworldpos.Y));
            }
            else if (customEntityType == typeof(PriestStart))
            {
                i = new PriestStart(currentbrush.fullpath, new Vector2((int)mouseworldpos.X, (int)mouseworldpos.Y));
            }
            else if (customEntityType == typeof(DoorItem))
            {
                i = new DoorItem(currentbrush.fullpath, new Vector2((int)mouseworldpos.X, (int)mouseworldpos.Y));
            }
            else if (customEntityType == typeof(FuseBoxItem))
            {
                i = new FuseBoxItem(currentbrush.fullpath, new Vector2((int)mouseworldpos.X, (int)mouseworldpos.Y));
            }
            else if (customEntityType == typeof(BlockerItem))
            {
                i = new BlockerItem(currentbrush.fullpath, new Vector2((int)mouseworldpos.X, (int)mouseworldpos.Y));
            }
            else if (customEntityType == null)
            {
                // Somwhere Here ( ZoidBerg )
        ...
        public void createPrimitiveBrush(PrimitiveType primitiveType, Type customEntityType, PolygonPath polygon = null)
        {
            this.customEntity = true;
            this.customEntityType = customEntityType;
            createPrimitiveBrush(primitiveType, polygon);
        }

        public void createPrimitiveBrush(PrimitiveType primitiveType, PolygonPath polygon = null)
        {
        ...
        public void paintPrimitiveBrush()
        {
            switch (currentprimitive)
            {
                ...
                case PrimitiveType.Circle:
                    ...
                    ci.Name = ci.getNamePrefix() + level.getNextItemNumber();// level.getNextItemNumber(); 
        ...
        private void HandlePolygonPath()
        {
            _currentPolygon.SetPolygon(clickedPoints.ToArray());
            _currentPolygon.Name = _currentPolygon.getNamePrefix() + level.getNextItemNumber();
            _currentPolygon.Visible = true;
            _currentPolygon = null;
            MainForm.Instance.toolStripStatusLabel1.Text = Resources.Path_Entered;
        ...
        public void loadLevel(Scene l)
        {
            ...
            foreach (Layer layer in l.Layers)
            {
                ...
                foreach (Item item in layer.Items)
                {
                    ...
                    // Set layers for drawable components
                    var gameObject = item as GameObjectItem;
                    if (gameObject != null)
                    {
                        foreach (var selectable in gameObject.GetSelectables())
        ...
        /// <summary>
        /// Updates the tree view and shows all available components in the Engine's Components folder.
        /// </summary>
        public void updateTreeViewComponents()
        {
            MainForm.Instance.contentTree.Nodes.Clear();
            var rootDirectoryInfo = new DirectoryInfo("../../../../Pontification/Components");

            // Recursively read components folder of the Engine.
            MainForm.Instance.contentTree.Nodes.Add(createDirectoryNode(rootDirectoryInfo));
        }

        private static Forms.TreeNode createDirectoryNode(DirectoryInfo directoryInfo)
        {
            var directoryNode = new Forms.TreeNode(directoryInfo.Name);
            ...
            {
                if (file.Name == "Component.cs")
                    continue;

                var node = new Forms.TreeNode(file.Name.Substring(0, file.Name.Length - 3));
                node.Tag = file.Name.Substring(0, file.Name.Length - 3);
