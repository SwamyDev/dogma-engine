...
namespace GLEED2D
{
    public partial class CircleItem
    {
       ...
       /* public override float getScale()
        ...
        {
            pRadius = (float)Math.Round(scale);
        }*/
        ...
        public override void drawInEditor(SpriteBatch sb)
        {
            if (!Visible) return;
            Color c = FillColor;
            if (hovering) c = Constants.Instance.ColorHighlight;
        ...
        public override void drawSelectionFrame(SpriteBatch sb, Matrix matrix, Color color)
        {
            ...
            {
                Primitives.Instance.drawBoxFilled(sb, p.X - 4, p.Y - 4, 8, 8, color);
            }

            Vector2 center = Vector2.Transform(pPosition, matrix);
            Primitives.Instance.drawBoxFilled(sb, center.X - 4, center.Y - 4, 8, 8, color);
