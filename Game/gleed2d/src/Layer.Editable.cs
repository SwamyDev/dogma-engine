...
namespace GLEED2D
{
    public partial class Layer
    {
        ...
        public Item getItemAtPos(Vector2 mouseworldpos)
        {
            ...
            {
                if (Items[i].contains(mouseworldpos) && Items[i].Visible) 
                    return Items[i];

                // Look for GameObject components we can manipulate
                var gameObject = Items[i] as GameObjectItem;
                if (gameObject != null)
                {
                    var selectables = gameObject.GetSelectables();
                    foreach (Item item in selectables)
                    {
                        if (item.contains(mouseworldpos) && item.Visible)
                        {
                            Editor.Instance.selectitem(gameObject);
                            return item;
                        }
                    }
                }
