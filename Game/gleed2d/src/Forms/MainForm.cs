...
namespace GLEED2D
{
    public partial class MainForm : Form
    {
        ...
        {
            Text = "PontEd - " + levelfilename + (DirtyFlag ? "*" : "");
        }
        ...
        private void treeView1_ItemDrag(object sender, ItemDragEventArgs e)
        {
            ...
            if (((TreeNode)e.Item).Tag is Scene) return;
        ...
        private void treeView1_DragOver(object sender, DragEventArgs e)
        {
            ...
            if (destnode == null || destnode.Tag is Scene) 
                return;
            ...
            {
                Item i1 = sourcenode.Tag as Item;
                if (i1 == null)
                    return;
        ...
        private void pictureBox1_DragEnter(object sender, DragEventArgs e)
        {
            ...
            Editor.Instance.createTextureBrush(lvi.Name,null);
        }
        ...
        {
            if (treeView1.SelectedNode.Tag is Scene)
            {
        ...
        private void ActionAddCustomProperty(object sender, EventArgs e)
        {
            ...
            if (treeView1.SelectedNode.Tag is Scene)
            {
                Scene l = (Scene)treeView1.SelectedNode.Tag;
        ...
        {
            Scene level = Scene.FromFile(filename, Game1.Instance.Content);
            ...
            if (level.EditorRelated.Version == null || level.EditorRelated.Version.CompareTo("1.3") < 0)
            {
                ...
                if (dr == DialogResult.OK)
                {
                    ...
                    //...and load it again
                    level = Scene.FromFile(filename, Game1.Instance.Content);
                }
                else
                {
                    return;
        ...
        private void propertyGrid1_PropertyValueChanged(object s, PropertyValueChangedEventArgs e)
        {
            ...
            // Check if we changed values for an animation object.
            if (e.ChangedItem.Parent != null)
            {
                var animation = e.ChangedItem.Parent.Value as AnimationItem;
                if (animation != null)
        ...
        private void listView1_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            if (itemtype == "file")
            {
                Editor.Instance.createTextureBrush(listView1.FocusedItem.Name,null);
            }
        ...
        private void listView1_ItemDrag(object sender, ItemDragEventArgs e)
        {
            ...
            propertyGrid1.DoDragDrop(e.Item, DragDropEffects.Copy);
        }
        ...
        public void loadfolder_foreground(string path)
        {
            ...
            string filters = "*.jpg;*.png;*.bmp;*.wav;*.wma;*.mp3";
            ...
            foreach (FileInfo file in files)
            {
                ...
                if (file.Extension == ".wav" || file.Extension == ".wma" || file.Extension == ".mp3")
                {
                    bmp = new Bitmap("Icons\\sound-icon.png");
        ...
        private void listView2_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            /*
             * Add Custom Primitives below
             */
            else if (listView2.FocusedItem.Text == "PlayerStart")
            {
                Editor.Instance.createTextureBrush("Assets/player.png", typeof(PlayerStart));
            }
            else if (listView2.FocusedItem.Text == "GuardStart")
            {
                Editor.Instance.createTextureBrush("Assets/guard.png", typeof(GuardStart));
            }
            else if (listView2.FocusedItem.Text == "PriestStart")
            {
                Editor.Instance.createTextureBrush("Assets/priest.png", typeof(PriestStart));
            }
            else if (listView2.FocusedItem.Text == "DoorItem")
            {
                Editor.Instance.createTextureBrush("Assets/door-closed.png", typeof(DoorItem));
            }
            else if (listView2.FocusedItem.Text == "BlockerItem")
            {
                Editor.Instance.createTextureBrush("Assets/statue.png", typeof(BlockerItem));
            }
            else if (listView2.FocusedItem.Text == "FuseBoxItem")
            {
                Editor.Instance.createTextureBrush("Assets/light-switch.png", typeof(FuseBoxItem));
            }
            else if (listView2.FocusedItem.Text == "LevelTransition")
            {
                Editor.Instance.createPrimitiveBrush(PrimitiveType.Rectangle, typeof(LevelTransition));
            }
            else if (listView2.FocusedItem.Text == "TurretItem")
            {
                Editor.Instance.createPrimitiveBrush(PrimitiveType.Rectangle, typeof(TurretItem));
            }
            else if (listView2.FocusedItem.Text == "HolyGround")
            {
                Editor.Instance.createPrimitiveBrush(PrimitiveType.Rectangle, typeof(HolyGround));
            }
            else if (listView2.FocusedItem.Text == "Water")
            {
                Editor.Instance.createPrimitiveBrush(PrimitiveType.Rectangle, typeof(Water));
            }
            else if (listView2.FocusedItem.Text == "HolySmoke")
            {
                Editor.Instance.createPrimitiveBrush(PrimitiveType.Rectangle, typeof(HolySmoke));
            }
            else if (listView2.FocusedItem.Text == "Trap")
            {
                Editor.Instance.createPrimitiveBrush(PrimitiveType.Rectangle, typeof(Trap));
            }
            else if (listView2.FocusedItem.Text == "AIPath")
            {
                Editor.Instance.createPrimitiveBrush(PrimitiveType.Path, typeof(AIPath));
            }
            else if (listView2.FocusedItem.Text == "IlluminationArea")
            {
                Editor.Instance.createPrimitiveBrush(PrimitiveType.Path, typeof(IlluminationArea));
            }
            else if (listView2.FocusedItem.Text == "PhysicsPath")
            {
                Editor.Instance.createPrimitiveBrush(PrimitiveType.Path, typeof(PhysicsPath));
            }
        }
        ...
        private void poSwitch_CheckedChanged_1(object sender, EventArgs e)
        {
            Editor.Instance.physicsItemMode = poSwitch.Checked;
        }

        // Light Object
        private void loSwitch_CheckedChanged(object sender, EventArgs e)
        {
            Editor.Instance.lightItemMode = loSwitch.Checked;
        }

        private void loSwitch_CheckedChanged_1(object sender, EventArgs e)
        {
            Editor.Instance.lightItemMode = loSwitch.Checked;
        }
        ...
        {
            System.Diagnostics.Process.Start("ProjectMercury.EffectEditor.exe");

        }

        private void loSwitch_CheckedChanged_2(object sender, EventArgs e)
        {
            Editor.Instance.lightItemMode = loSwitch.Checked;
        }

        private void exportPhysicsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            FolderBrowserDialog selectExportFolder = new FolderBrowserDialog();

            if (selectExportFolder.ShowDialog() == DialogResult.OK)
            {
                Game1.Instance.ExportPhysicsPathsToPNG(selectExportFolder.SelectedPath);
            }
        }

        private void pictureBox1_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == System.Windows.Forms.MouseButtons.Right && Editor.Instance.state == EditorState.idle)
            {
                // Show greate game object context menu when right clicking.
                var relativeMousePos = pictureBox1.PointToClient(Cursor.Position);
                CanvasContextMenu.Show(pictureBox1, relativeMousePos);
            }
        }

        private void addGameObjectToolStripMenuItem_Click(object sender, EventArgs e)
        {
            // Add a new game object at the specified position.
            Editor.Instance.AddGameObject();
        }
        ...
        {
            // Drop component to game object
            var go = propertyGrid1.SelectedObject as GameObjectItem;
            if (go != null)
            {
                TreeNode sourceNode = e.Data.GetData(typeof(TreeNode)) as TreeNode;

                #region Add component to game object
                if (sourceNode != null)
                {
                    var componentType = Type.GetType(string.Format("Pontification.Components.{0}, Pontification, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null", sourceNode.Tag));
                    var component = Activator.CreateInstance(componentType) as Pontification.Components.Component;
                    var propertyBag = new PropertyBag(componentType.Name, true);

                    // Get public properties of the dropped component and add it to the propertyBag which is used to display the properties in the propertyGrid
                    foreach (var prop in componentType.GetProperties(BindingFlags.Public | BindingFlags.Instance))
                    {
                        if (prop.Name == "GameObject")
                            continue;

                        propertyBag.SetProperty(prop.Name, getPropertyValue(prop, go, component));
                    }
                    go.Components.Add(propertyBag);
                ...
                #region Add texture/sound information to component
                if (lvi != null)
                {
                    object propertyGridView = GetPropertyGridView(propertyGrid1);
                    GridItemCollection allGridItems = GetAllGridEntries(propertyGridView);
                    int top = GetTopOfGridView(propertyGridView);
                    int itemHeight = GetCachedRowHeight(propertyGridView);
                    VScrollBar scrollBar = GetVScrollBar(propertyGridView);

                    GridItem item = GetGridItemAtPoint(allGridItems, top, itemHeight, scrollBar.Value, propertyGrid1.PointToClient(new Point(e.X, e.Y)));
                    string fileExtension = lvi.Name.Substring(lvi.Name.Length - 3);

                    if (fileExtension == "wav" || fileExtension == "wma" || fileExtension == "mp3")
                    {
                        var soundData = item.Value as SoundDataItem;
                        if (soundData != null)
                        {
                            var path = Scene.RelativePath(Editor.Instance.level.ContentRootFolder, lvi.Name);
                            soundData.AssetName = path.Substring(0, path.LastIndexOf('.'));
                            propertyGrid1.Refresh();
                        }
                    }
                    else
                    {
                        var textureItem = item.Value as TextureItem;
                        if (textureItem != null)
                        {
                            // Drop texture to item.
                            textureItem.texture_filename = Scene.RelativePath(textureItem.layer.level.ContentRootFolder, lvi.Name);
                            textureItem.asset_name = textureItem.texture_filename.Substring(0, textureItem.texture_filename.LastIndexOf('.'));
                            textureItem.Visible = true;
                            if (textureItem.loadIntoEditor())
                            {
                                textureItem.centerTexture();
                            }
                            propertyGrid1.Refresh();
                        }
                    }
            ...
            contentTree.Cursor = Cursors.Default;
            propertyGrid1.Cursor = Cursors.Default;
        }

        private object getPropertyValue(PropertyInfo prop, GameObjectItem go, object component)
        {
            ...
            else if (prop.PropertyType.GetInterface("IEnumerable`1") != null && prop.PropertyType.GetGenericArguments()[0].FullName == "Microsoft.Xna.Framework.Vector2")
            {
                // We have a Vector2 collection here. This means we need to create a polygn path to fill them.
                return PolygonPath.CreateEmpty(go);
            }
            else if (prop.PropertyType.GetInterface("IDictionary`2") != null)
            {
                // For dictionaries just add another ProeprtyBag which holds key value pairs for the dictionary.
                var dictionaryBag = new PropertyBag(prop.Name, false);

                var keyType = prop.PropertyType.GetGenericArguments()[0];
                dictionaryBag.TypeName = keyType.Name;

                // If we have Enum as keys create a property for each enum
                if (keyType.IsEnum)
                {
                    var enumNames = keyType.GetEnumNames();
                    foreach (var enumName in enumNames)
                        dictionaryBag.SetProperty(enumName, "");
                }

                return dictionaryBag;
            }
            else if (prop.PropertyType.GetInterface("IEnumerable`1") != null)
            {
                // We got a list here.
                var itemType = prop.PropertyType.GetGenericArguments()[0];
                if (itemType == typeof(IEnumerable<Microsoft.Xna.Framework.Vector2>))
                    itemType = typeof(PolygonPath);
                else if (itemType == typeof(Dictionary<,>))
                    itemType = typeof(PropertyBag);
                else if (itemType == typeof(Texture2D))
                    itemType = typeof(TextureItem);
                else if (itemType == typeof(Pontification.Animation))
                    itemType = typeof(AnimationItem);
                else if (!itemType.IsValueType)
                    itemType = typeof(string);

                var openType = typeof(ListItem<>);
                var listType = openType.MakeGenericType(itemType);

                return Activator.CreateInstance(listType);
            }
            else if (prop.PropertyType == typeof(Texture2D))
            {
                return TextureItem.CreateEmpty(go);
            }
            else if (prop.PropertyType == typeof(Pontification.Animation))
            {
                return AnimationItem.CreateEmpty(go);
            }
            else if (prop.PropertyType == typeof(Pontification.SoundEffects))
            {
                return SoundEffectsItem.CreateEmpty(go);
            }
            else if (prop.PropertyType == typeof(Pontification.GameObject))
            {
        ...
        {
            // If we have a polygon path as value start brush.
            var polygon = propertyGrid1.SelectedGridItem.Value as PolygonPath;
            if (polygon != null && (polygon.WorldPoints == null || polygon.WorldPoints.Length == 0))
                Editor.Instance.createPrimitiveBrush(PrimitiveType.Path, polygon);
        }

        #region Drag and drop to property grid item HACK
        private object GetPropertyGridView(PropertyGrid propertyGrid)
        {
        ...
        private GridItemCollection GetAllGridEntries(object propertyGridView)
        {
        ...
        private int GetTopOfGridView(object propertyGridView)
        {
            Control c = (Control)propertyGridView;

            return c.Top;
        }

        private int GetCachedRowHeight(object propertyGridView)
        {
        ...
        private VScrollBar GetVScrollBar(object propertyGridView)
        {
        ...
        private GridItem GetGridItemAtPoint(GridItemCollection items, int top, int itemHeight, int scrollItems, Point pt)
        {
        ...
        private void timerLoad_Tick(object sender, EventArgs e)
        {
            ...
            Application.AddMessageFilter(new PropertyGridMessageFilter(propertyGrid1.GetChildAtPoint(new Point(40, 40)),
                new MouseEventHandler(propGridView_MouseUp)));
        }

        private void propGridView_MouseUp(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right && propertyGrid1.SelectedGridItem != null)
            {
                // the user right clicked on a property to see the context menu: 
                var propertyBag = propertyGrid1.SelectedGridItem.Value as PropertyBag;
                if (propertyBag != null)
                {
                    if (propertyBag.IsComponent)
                        componentContextMenu.Show(MousePosition);
                    else
                        dictPropertyContextMenu.Show(MousePosition);
                }
                else if (propertyGrid1.SelectedGridItem.Value is PolygonPath)
                {
                    polygonContextMenu.Show(MousePosition);
                }
                else if (propertyGrid1.SelectedGridItem.Value is AnimationItem)
                {
                    animationSelection((AnimationItem)propertyGrid1.SelectedGridItem.Value);
                }
                else if (propertyGrid1.SelectedGridItem.Value.GetType().Name == "ListItem`1")
                {
                    AddItemToList(propertyGrid1.SelectedGridItem.Value);
                }
                else if (propertyGrid1.SelectedGridItem.Parent != null)
                {   // If we are within a custom dictionary gove the oportunity to delete items.
                    var parentBag = propertyGrid1.SelectedGridItem.Parent.Value as PropertyBag;
                    if ((parentBag != null && !parentBag.IsComponent) || propertyGrid1.SelectedGridItem.Parent.Value.GetType().Name == "ListItem`1")
                    {
                        customDictPropCntxMenu.Show(MousePosition);
                    }
                }
            }
        }
        ...
        private void animationSelection(AnimationItem animation)
        {
            animation.UpdateAnimation();
            MenuItem[] menuItems = new MenuItem[animation.AnimationData.Count + 1];

            for (int i = 0; i < menuItems.Length - 1; i++)
            {
                menuItems[i] = new MenuItem(animation.AnimationData[i].Name, new EventHandler(selectedAnimation_Click));
                menuItems[i].Tag = animation;
            }
            menuItems[animation.AnimationData.Count] = new MenuItem("None", new EventHandler(selectedAnimation_Click));
            menuItems[animation.AnimationData.Count].Tag = animation;

            var ctxMenu = new ContextMenu(menuItems);
            ctxMenu.Show(propertyGrid1, propertyGrid1.PointToClient(MousePosition));
        }

        private void selectedAnimation_Click(object sender, EventArgs e)
        {
            var menuItem = sender as MenuItem;

            if (menuItem != null)
            {
                var animation = menuItem.Tag as AnimationItem;
                if (animation != null)
                {
                    animation.PlayAnimation(menuItem.Text);
                }
            }
        }
        #endregion

        #region Add list item
        private void AddItemToList(object list)
        {
            var menuItem = new MenuItem("Add item", new EventHandler(addItem_Click));
            menuItem.Tag = list;

            var ctxMenu = new ContextMenu(new MenuItem[] { menuItem });
            ctxMenu.Show(propertyGrid1, propertyGrid1.PointToClient(MousePosition));
        }
        ...
        private void addItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var go = propertyGrid1.SelectedObject as GameObjectItem;
                var listType = menuItem.Tag.GetType();
                var itemType = listType.GetGenericArguments()[0];
                var list = Convert.ChangeType(menuItem.Tag, listType);

                // Generate empty parameter.
                object value = null;

                if (itemType == typeof(PolygonPath))
                {
                    value = PolygonPath.CreateEmpty(go); 
                }
                else if (itemType == typeof(PropertyBag))
                {
                    value = new PropertyBag();
                }
                else if (itemType == typeof(TextureItem))
                {
                    value = TextureItem.CreateEmpty(go);
                }
                else if (itemType == typeof(AnimationDataItem))
                {
                    value = new AnimationDataItem();
                }
                else if (itemType == typeof(AnimationItem))
                {
                    value = AnimationItem.CreateEmpty(go);
                }
                else if (itemType == typeof(SoundDataItem))
                {
                    value = new SoundDataItem();
                }
                else if (itemType == typeof(SoundEffectsItem))
                {
                    value = SoundEffectsItem.CreateEmpty(go);
                }
                else if (itemType.IsValueType)
        ...
        {
            // Remove currently selected component from game object.
            var comp = propertyGrid1.SelectedGridItem.Value as PropertyBag;
            var go = propertyGrid1.SelectedObject as GameObjectItem;
            ...
            if (comp != null && go != null)
            {
                ...
                propertyGrid1.Refresh();
                go.Components.Remove(comp);
                propertyGrid1.SelectedObject = go;
                propertyGrid1.Refresh();
            }
        }

        private void redrawPolygonToolStripMenuItem_Click(object sender, EventArgs e)
        {
            var polygon = propertyGrid1.SelectedGridItem.Value as PolygonPath;
            var go = propertyGrid1.SelectedObject as GameObjectItem;

            if (polygon != null && go != null)
            {
                // polygon.SetPolygon(new Microsoft.Xna.Framework.Vector2[0]);
                Editor.Instance.createPrimitiveBrush(PrimitiveType.Path, polygon);
            }
        }

        #region Add properties to dictionary variable
        private void addPropertyToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (propertyBag != null)
            {
                ...
                t.Text = "";
                t.Tag = propertyBag; // so we know what property is about 
                t.KeyUp += new KeyEventHandler(textbox_name_KeyUp); // to capture Enter & Escape 
                t.MouseUp += new MouseEventHandler(textbox_name_MouseUp); // to allow hidding of the textbox by clicking somewhere else 
                t.Capture = true; // i want all the mouse messages to be sent to the textbox 

                // placing the textbox in the form, right under the mouse: 
                Point mp = MousePosition; mp.Offset(-3, -3); t.Location = PointToClient(mp);
                Controls.Add(t);
                t.BringToFront();
                t.Focus();

                t.Select(t.Text.Length, 0); // clears the initial selection of the text 
            ...
            switch (e.KeyData)
            {
                case Keys.Enter:
                    {// keep the changes: 
                        ...
                        string propName = t.Text != "" ? t.Text : "must type something or the control will go berserk";
                        ((PropertyBag)t.Tag).SetProperty(propName, "");

                        RemoveTextBox(sender);

                        propertyGrid1.Refresh(); // refresh the grid since it can't possibly know we changed something 
                    } break;
        ...
        private void textbox_name_MouseUp(object sender, MouseEventArgs e)
        {
            Control c = sender as Control;

            if (!c.RectangleToScreen(c.ClientRectangle).Contains(MousePosition))
            {
                // the user clicked somewhere else, so the textbox must disappear: 
                RemoveTextBox(sender);
            }
        }

        /// <summary>
        /// Does all the necessary actions to remove the provided control (textbox) from the form
        /// </summary>
        /// <param name="sender"></param>
        public void RemoveTextBox(object sender)
        {
            Control c = sender as Control;
            c.Capture = false; // we captured the mouse events, remember ? 
            if (Controls.Contains(c)) // to be safe 
                Controls.Remove(c);
            c.Dispose(); // always dispose controls 
        }
        #endregion

        private void removePropertyToolStripMenuItem_Click(object sender, EventArgs e)
        {
            var propertyName = propertyGrid1.SelectedGridItem.Label;
            var comp = propertyGrid1.SelectedGridItem.Parent.Value as PropertyBag;
            var go = propertyGrid1.SelectedObject as GameObjectItem;
            ...
            if (go != null && comp != null)
            {
                propertyGrid1.SelectedObject = null;
                propertyGrid1.Refresh();
                comp.RemoveProperty(propertyName);
                propertyGrid1.SelectedObject = go;
                propertyGrid1.Refresh();
            }
            else if (go != null)
            {
                var list = propertyGrid1.SelectedGridItem.Parent.Value;
                var value = propertyGrid1.SelectedGridItem.Value;
                var method = list.GetType().GetMethod("Remove");
                propertyGrid1.SelectedObject = null;
                propertyGrid1.Refresh();
                method.Invoke(list, new object[] { value });
                propertyGrid1.SelectedObject = go;
